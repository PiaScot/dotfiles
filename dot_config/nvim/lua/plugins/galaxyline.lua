		...
		table.insert(gls.right, {
			FileName = {
				provider = function()
					...
					local fname = vim.fn.expand("%:p")
					local truncated_fname = fname:gsub(vim.fn.getcwd() .. "/", "")

					local parts = {}
					for part in string.gmatch(truncated_fname, "[^/]+") do
						table.insert(parts, part)
					end

					if #parts > 3 then
						truncated_fname = table.concat(parts, "/", #parts - 2)
					end

					return truncated_fname .. " "
				end,
