	...
	config = function()
		...
		local function get_linter(bufnr)
			...
			if
				vim.fs.find({
					...
					".eslintrc.json",
					"eslint.config.js",
				}, { path = buf_dir, upward = true, stop = vim.uv.os_homedir() })[1]
			then
				return { "eslint_d" }
			end
			if vim.fs.find({ "biome.json" }, { path = buf_dir, upward = true, stop = vim.uv.os_homedir() })[1] then
				return { "biome" }
			end
		...
		vim.api.nvim_create_autocmd({ "BufWritePost" }, {
			callback = function(args)
				...
				if
					not lint_module
					or type(lint_module) ~= "table"
					or not lint_module.set_linters
					or not lint_module.try_lint
				then
					-- lint module not fully loaded or functions not available yet, skip
					return
				end

				local filetype = vim.api.nvim_get_option_value(args.buf, filetype)
